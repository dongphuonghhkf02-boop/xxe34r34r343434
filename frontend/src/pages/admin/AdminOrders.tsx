  ...
  return (
    <div className={styles.wrap} data-testid="admin-sales-orders">
      <div className={styles.toolbar}>
        ...
        />
        <select className={styles.select} value={payStatus} onChange={(e) => setPayStatus(e.target.value as any)} data-testid="orders-filter-payment">
          {PAYMENT_FILTERS.map((f) => <option key={f.v} value={f.v}>{f.label}</option>)}
        </select>
        <select className={styles.select} value={intStatus} onChange={(e) => setIntStatus(e.target.value as any)} data-testid="orders-filter-internal">
          {INTERNAL_FILTERS.map((f) => <option key={f.v} value={f.v}>{f.label}</option>)}
        </select>
        <select className={styles.select} value={method} onChange={(e) => setMethod(e.target.value as any)} data-testid="orders-filter-method">
          {METHOD_FILTERS.map((f) => <option key={f.v} value={f.v}>{f.label}</option>)}
        </select>
